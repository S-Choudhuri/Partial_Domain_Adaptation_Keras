...
def train(param):
    models = {}
    inp = Input(shape = (param["inp_dims"]), name ='inp')
    sinp = Input(shape = (param["inp_dims"]), name ='sinp')
    tinp = Input(shape = (param["inp_dims"]), name ='tinp')
    embedding = model.build_embedding(param, inp,'embedding')
    s_embedding = model.build_embedding(param, sinp,'sembedding')
    t_embedding = model.build_embedding(param, tinp,'tembedding')
    classifier = model.build_classifier(param, embedding)
    discriminator = model.build_discriminator(param, embedding)
    s_decoding = model.build_decoder(param, embedding, s_embedding, 'sdec')
    t_decoding = model.build_decoder(param, embedding, t_embedding, 'tdec')
    ...
    if param["number_of_gpus"] > 1:
        ...
        models["s_enc_dec"] = multi_gpu_model(model.build_enc_dec(inp, sinp, s_decoding, 'sencdec'), gpus = param["number_of_gpus"])
        models["t_enc_dec"] = multi_gpu_model(model.build_enc_dec(inp, tinp, t_decoding, 'tencdec'), gpus = param["number_of_gpus"])
    else:
        ...
        models["s_enc_dec"] = model.build_enc_dec(inp, sinp, s_decoding, 'sencdec')
        models["t_enc_dec"] = model.build_enc_dec(inp, tinp, t_decoding, 'tencdec')
    ...
    models["s_enc_dec"].compile(optimizer = optimizer.opt_classifier(param), loss = 'mae', metrics = ['mae'])
    models["t_enc_dec"].compile(optimizer = optimizer.opt_classifier(param), loss = 'mae', metrics = ['mae'])
    ...
    for i in range(param["num_iterations"]):        
        ...
        for layer in models["combined_model"].layers:
            if (not layer.name.startswith("dis_")):
                ...
                k += 1
        print(Xsb.shape, int(weight_adv.shape[0]/2))
        stats3 = models["s_enc_dec"].train_on_batch([Xsb,Xsb], Xsb,\
                 				sample_weight = [weight_adv[0:int(weight_adv.shape[0]/2)]])
        stats4 = models["t_enc_dec"].train_on_batch([Xtb,Xtb], Xtb)                   
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_interval', type = int, default = 10, help = "Gap between two successive test phases")
